...
In order to run this backend API locally, the main requirements are [Node.js](https://nodejs.org/), [npm](https://www.npmjs.com/get-npm) and a [MongoDB](https://www.mongodb.com/) instance (either local or cloud-based). You can then follow these steps :
...
Although I intended to use an ORM, I thought that I had to give a try to [MongoDB](https://www.mongodb.com/) first. Being the most common solution amongst Node stacks, it seemed important to have some working knowledge of it.
...
Another issue I spent some time on was finding a way to make my tests wait until the app was ready. At that point, the only async process I had to wait for was the [mongoose](https://mongoosejs.com/) connection. I first tried propagating a callback from the app to my tests, but it felt wrong and messy. Then I remembered events, something I'm was definitely not used to. The issue then was that it worked only for the first test as the event was only fired once, and also that it could theoretically happen before registering my listener. So I added a *isReady* property to my [Express](https://expressjs.com/) app, which was probably something not so great even if I checked for conflicts first, that I would evaluate first to determine if the event had already been fired.
...
There were times when I stumbled upon mistakes in my integration tests, which was no surprise given my lack of experience to say the least. But there were also times when I realized some tests were not thorough enough, or even worse, actually not testing what was described and displayed a deceitful green. For instance, this time tracking application was meant to handle time segments and had a key constraint which is that two of those segments, called activities, could not overlap. In other words, a user could not have two activities at the same time, making mono-tasking mandatory. It might have seemed simple and obvious, but it meant a lot more checks in my test suite that I had anticipated. I had clearly not given it enough thought and made a note to self to go more in-depth next time.
...
The remaining features, in both tests and implementations, were done in a much shorted time than I expected. I was really eager to do some kind of feature freeze and a final code review. Something I noticed, which I took for a good sign, is that I had not been reading tutorials on Medium or elsewhere for quite some time. It had only been my [IDE](https://en.wikipedia.org/wiki/Integrated_development_environment), libraries docs and the occasional StackOverflow visits. They helped me to get started but I had progressively build enough confidence to pursue my own ideas.

Anyway, this Node.js API was *far* from any perfection, but I was quite satisfied with how much I had learned along the way. It had been such a big leap for me in terms of paradigm. The language I had had the most knowledge and experience with had been Java, which is class-based OOP and strongly typed, whereas JavaScript is prototype-based OOP and weakly typed. Some say that Java is to JavaScript what ham is to hamster. And the only web framework I had worked with before was [Rails](https://rubyonrails.org/), very opinionated, with embedded generators and scaffolding and advocating convention over configuration, whereas [Express](https://expressjs.com/) is *quite* the opposite as it is very minimalist and mainly handles routing and middleware stacking. All those concepts were unknown to me only a few weeks back, and now seemed so familiar.

### What's next

With a general understanding of how Node.js servers work and a basic API to work with, it was time to dive into the realm of front-end JavaScript frameworks and continue my [coding journey](https://github.com/stoneLeaf/coding-journey). I will probably come back to this API to fix bugs and add features I might deem necessary.
